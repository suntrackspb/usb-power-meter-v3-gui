...
На Linux уберите `--icon` (на этой платформе флаг PyInstaller ничего не
делает; иконка подхватывается из `assets/icon.png` во время выполнения). На
Windows используйте `--icon assets\icon.ico` и `--add-data "web;web"
